...
const ConfigGlobalForm: React.FC = () => {
  ...
  const [originalConfig, setOriginalConfig] = useState<ConfigGlobal | null>(
    null
  );
  ...
  const loadConfig = async (): Promise<void> => {
    ...
    try {
      const res = await axios.get(
        "http://localhost:3000/configuration/configuracion-global"
      );
  ...
  const handleChange = (
    ...
    if (name === "nombreEmpresa" && !value.trim()) {
      setErrors((prev) => ({
        ...prev,
        nombreEmpresa: "El nombre de la empresa es obligatorio",
      }));
    ...
    if (name === "textoPrincipal" && value.length > 500) {
      setErrors((prev) => ({
        ...prev,
        textoPrincipal: "El texto principal no debe superar los 500 caracteres",
      }));
  ...
  const handleColorChange = (
    ...
    if (field === "backgroundDark") {
      const textosCompatibles =
        CONTRASTE_SUFICIENTE[value as keyof typeof CONTRASTE_SUFICIENTE] || [];
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (file && config) {
      ...
      if (!validTypes.includes(file.type)) {
        alert(
          "Por favor, seleccione un archivo de imagen válido (JPEG, PNG o SVG)"
        );
        return;
      }
      ...
      setLogoFile(file);
      // Solo guardar el nombre del archivo, sin crear una URL local
      setConfig({ ...config, logoUrl: `/${file.name}` });
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (config.textoPrincipal && config.textoPrincipal.length > 500) {
      newErrors.textoPrincipal =
        "El texto principal no debe superar los 500 caracteres";
      isValid = false;
    }
  ...
  const ColorSelector: React.FC<ColorSelectorProps> = ({
    ...
    const colors =
      colorType === "text" ? colorPalette.text : colorPalette.backgrounds;
    ...
    return (
      <div className="space-y-2">
          ...
          {colors.map((color) => {
            ...
            const isCompatible =
              ...
              !CONTRASTE_SUFICIENTE[
                config?.backgroundDark as keyof typeof CONTRASTE_SUFICIENTE
              ] ||
              CONTRASTE_SUFICIENTE[
                config?.backgroundDark as keyof typeof CONTRASTE_SUFICIENTE
              ].includes(color.class);

            const isDisabled =
              disabled || (colorType === "text" && !isCompatible);
            ...
            return (
              <div
                ...
                title={`${color.name}${
                  !isCompatible
                    ? " (No compatible con el fondo seleccionado)"
                    : ""
                }`}
              >
                {value === color.class && (
        ...
        <div className="mt-1 flex justify-between items-center">
          ...
          {helperText && (
            <span className="text-xs text-blue-600">{helperText}</span>
          )}
        </div>
      </div>
    );
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={`${config.backgroundWhite} rounded-lg p-6 shadow`}>
              <div className="font-medium">
                Fondo claro ({config.backgroundWhite})
              </div>
              <p className="mt-2">Ejemplo de texto en este fondo</p>
            </div>
            <div className={`${config.backgroundGray} rounded-lg p-6 shadow`}>
              <div className="font-medium">
                Fondo gris ({config.backgroundGray})
              </div>
              <p className="mt-2">Ejemplo de texto en este fondo</p>
            </div>
            <div
              className={`${config.backgroundDark} ${config.textLight} rounded-lg p-6 shadow`}
            >
              <div className="font-medium">
                Fondo oscuro ({config.backgroundDark})
              </div>
